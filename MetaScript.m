...
tau = 0.01;
dt = .0001;
t_final = 100.*tau;  % Creates a range for the time
...
C_plus = 0.5;
C_minus = -1.05.*C_plus;
lambda = 100;			% temporary value, change according to literature
...
%     [ V_B(currentTime+1), binary_spike_B(currentTime+1) ] = neuronB(V_B(currentTime));
    ...
    if binary_spike_S(currentTime) == 1 && A_spikeTime ~= 0;
        deltaT_A = (A_spikeTime - currentTime).*dt;
    ...
    elseif binary_spike_A(currentTime) == 1 && S_spikeTime ~= 0;
        deltaT_A = -(S_spikeTime - currentTime).*dt;
    ...
    if binary_spike_S(currentTime) == 1 && B_spikeTime ~= 0;
        deltaT_B = (B_spikeTime - currentTime).*dt;
    ...
    elseif binary_spike_B(currentTime) == 1 && S_spikeTime ~= 0;
        deltaT_B = -(S_spikeTime - currentTime).*dt;
...
% Plots
figure(1)
plot(time_global, V_A, time_global, V_B, time_global, V_T, time_global, V_S)
legend('V_A', 'V_B', 'V_T', 'V_S')
xlabel('Time (s)'), ylabel('Membrane Voltage (V)')
title('Membrane Voltage of Four Neurons over Time')
%
figure(2)
