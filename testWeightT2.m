function [ scenario ] = testWeightT2( sigmoidX_T, alpha_T_in )
...
W_T(1) = W_max ./ (1 + exp(-alpha_T.*sigX_T));

% NEW CODE: Fix final weight of T to be 0.1 and find corresponding final X
% Rearrange sigmoid to solve for X
W_T_final = 0.1;
XWT_final = -(1./4).*log( W_max./W_T_final - 1 );

delta_XWT = ( sigX_T - XWT_final )./ numel( time_global );
XWT = sigX_T;
...
%     [ V_B(currentTime+1), binary_spike_B(currentTime+1) ] = neuronB(V_B(currentTime));
    ...
    [ V_S(currentTime), Ie_S(currentTime), binary_spike_S(currentTime) ] = ...
        neuronS( V_S(currentTime-1), Ie_S(currentTime-1), ...
        binary_spike_A(currentTime-1), binary_spike_B(currentTime-1), ...
        binary_spike_T(currentTime-1), W_A(currentTime-1), W_B(currentTime-1), W_T(currentTime-1) );
...
%     W_T(currentTime) = W_T(currentTime-1);
    ...
    % For trials that include atrophy of the teacher
    XWT = XWT - delta_XWT;
    W_T(currentTime) = W_max ./ (1 + exp(-alpha_T.*XWT));
